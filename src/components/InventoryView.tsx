...
export const InventoryView: React.FC<InventoryViewProps> = ({
  ...
  // Extract brands and categories dynamically
  ...
  const filteredProducts = useMemo(() => {
    return products.filter(p => {
      ...
      const matchesStock = stockFilter === 'all' ? true :
                           ...
                           stockFilter === 'low' ? p.stock > 0 && p.stock <= 4 :
                           p.stock === 0;

      return matchesBrand && matchesCategory && matchesSearch && matchesStock;
    });
  }, [products, selectedBrand, selectedCategory, searchQuery, stockFilter]);
  ...
  const handleAddProduct = (e: React.FormEvent) => {
    ...
    const newProduct: Product = {
      ...
      specs: newProductSpecs || 'Standard specifications',
      color: newProductColor || 'Midnight',
      imeiRequired: newProductImei
    };
  ...
  const resetForm = () => {
    setNewProductName('');
    setNewProductBrand('Apple');
    setNewProductCategory('Smartphones');
    setNewProductPrice('');
    setNewProductStock('');
    setNewProductSpecs('');
    setNewProductColor('');
    setNewProductImei(true);
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
        ...
        <div className="lg:col-span-3 flex flex-wrap gap-4 items-center bg-white p-4 rounded-2xl border border-slate-100 shadow-sm">
          ...
          <div className="flex items-center gap-2 px-3 py-1.5 bg-slate-50 rounded-lg border border-slate-100 text-slate-500 font-semibold text-xs uppercase tracking-wider font-mono">
            ...
            {selectedCategory === 'All Categories' ? t('pos.allCategories') : selectedCategory}
          </div>

          {/* Search bar inside filter layout */}
          ...
          >
            {brands.map(b => (
              <option key={b} value={b}>{b === 'All Brands' ? t('pos.allCategories') : b}</option>
            ))}
          </select>
          ...
          >
            {categories.map(c => (
              <option key={c} value={c}>{c === 'All Categories' ? t('pos.allCategories') : c}</option>
            ))}
          </select>
                ...
                <tr key={p.id} className="hover:bg-slate-50/60 transition-colors group">
                  ...
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-4">
                      <div className="w-12 h-12 rounded-xl bg-slate-50 border border-slate-100 overflow-hidden flex-shrink-0 p-1.5 flex items-center justify-center">
                        <img className="h-full object-contain" src={p.image} alt={p.name} referrerPolicy="no-referrer" />
                      </div>
                      <div>
                        <p className="font-bold text-slate-900 text-sm">{p.name}</p>
                        <p className="text-xs text-slate-500 mt-0.5 font-mono">{p.specs} • ID: {p.id}</p>
                      </div>
                    </div>
                  </td>

                  {/* Brand */}
                  <td className="px-6 py-4 text-slate-600 text-sm font-semibold">{p.brand}</td>
                  
                  {/* Category */}
                  <td className="px-6 py-4 text-slate-600 text-sm">{p.category}</td>
                  
                  {/* Price */}
                  <td className="px-6 py-4 text-right font-bold text-primary text-sm font-mono">${p.price.toLocaleString('en', { minimumFractionDigits: 2 })}</td>
